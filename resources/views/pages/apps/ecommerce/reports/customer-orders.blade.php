@extends('layouts.index')
@section('styles')
    <!--begin::Vendor Stylesheets(used for this page only)-->
    <link href="{{ \App\Support\ThemeAsset::url('plugins/custom/datatables/datatables.bundle.css', $theme_asset_pack ?? null) }}" rel="stylesheet" type="text/css" />
    <!--end::Vendor Stylesheets-->
@endsection
@section('toolbar')
    @component('layouts.partials._toolbar')
        @slot('li_1')
            Apps
        @endslot
        @slot('li_2')
            eCommerce
        @endslot
        @slot('li_3')
            Reports
        @endslot
    @endcomponent
@endsection
@section('content')
    <!--begin::Content-->
    <div id="kt_app_content" class="app-content flex-column-fluid">
        <!--begin::Content container-->
        <div id="kt_app_content_container" class="app-container container-fluid">
            <!--begin::Products-->
            <div class="card card-flush">
                <!--begin::Card header-->
                <div class="card-header align-items-center py-5 gap-2 gap-md-5">
                    <!--begin::Card title-->
                    <div class="card-title">
                        <!--begin::Search-->
                        <div class="d-flex align-items-center position-relative my-1">
                            <i class="ki-duotone ki-magnifier fs-3 position-absolute ms-4">
                                <span class="path1"></span>
                                <span class="path2"></span>
                            </i>
                            <input type="text" data-kt-ecommerce-order-filter="search"
                                class="form-control form-control-solid w-250px ps-12" placeholder="Search Report" />
                        </div>
                        <!--end::Search-->
                        <!--begin::Export buttons-->
                        <div id="kt_ecommerce_report_customer_orders_export" class="d-none"></div>
                        <!--end::Export buttons-->
                    </div>
                    <!--end::Card title==

                                                <!==begin::Card toolbar-->
                    <div class="card-toolbar flex-row-fluid justify-content-end gap-5">
                        <!--begin::Daterangepicker-->
                        <input class="form-control form-control-solid w-100 mw-250px" placeholder="Pick date range"
                            id="kt_ecommerce_report_customer_orders_daterangepicker" />
                        <!--end::Daterangepicker-->
                        <!--begin::Filter-->
                        <div class="w-150px">
                            <!--begin::Select2-->
                            <select class="form-select form-select-solid" data-control="select2" data-hide-search="true"
                                data-placeholder="Status" data-kt-ecommerce-order-filter="status">
                                <option></option>
                                <option value="all">All</option>
                                <option value="active">Active</option>
                                <option value="locked">Locked</option>
                                <option value="disabled">Disabled</option>
                                <option value="banned">Banned</option>
                            </select>
                            <!--end::Select2-->
                        </div>
                        <!--end::Filter-->
                        <!--begin::Export dropdown-->
                        <button type="button" class="btn btn-light-primary" data-kt-menu-trigger="click"
                            data-kt-menu-placement="bottom-end">
                            <i class="ki-duotone ki-exit-up fs-2">
                                <span class="path1"></span>
                                <span class="path2"></span> </i>Export Report
                        </button>
                        <!--begin::Menu-->
                        <div id="kt_ecommerce_report_customer_orders_export_menu"
                            class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-600 menu-state-bg-light-primary fw-semibold fs-7 w-200px py-4"
                            data-kt-menu="true">
                            <!--begin::Menu item-->
                            <div class="menu-item px-3">
                                <a href="javascript:void(0)" class="menu-link px-3" data-kt-ecommerce-export="copy">Copy to
                                    clipboard</a>
                            </div>
                            <!--end::Menu item-->
                            <!--begin::Menu item-->
                            <div class="menu-item px-3">
                                <a href="javascript:void(0)" class="menu-link px-3" data-kt-ecommerce-export="excel">Export as
                                    Excel</a>
                            </div>
                            <!--end::Menu item-->
                            <!--begin::Menu item-->
                            <div class="menu-item px-3">
                                <a href="javascript:void(0)" class="menu-link px-3" data-kt-ecommerce-export="csv">Export as CSV</a>
                            </div>
                            <!--end::Menu item-->
                            <!--begin::Menu item-->
                            <div class="menu-item px-3">
                                <a href="javascript:void(0)" class="menu-link px-3" data-kt-ecommerce-export="pdf">Export as PDF</a>
                            </div>
                            <!--end::Menu item-->
                        </div>
                        <!--end::Menu-->
                        <!--end::Export dropdown-->
                    </div>
                    <!--end::Card toolbar-->
                </div>
                <!--end::Card header-->
                <!--begin::Card body-->
                <div class="card-body pt-0">
                    <!--begin::Table-->
                    <table class="table align-middle table-row-dashed fs-6 gy-5"
                        id="kt_ecommerce_report_customer_orders_table">
                        <thead>
                            <tr class="text-start text-gray-500 fw-bold fs-7 text-uppercase gs-0">
                                <th class="min-w-100px">Customer Name</th>
                                <th class="min-w-100px">Email</th>
                                <th class="min-w-100px">Status</th>
                                <th class="min-w-100px">Date Joined</th>
                                <th class="text-end min-w-75px">No. Orders</th>
                                <th class="text-end min-w-75px">No. Products</th>
                                <th class="text-end min-w-100px">Total</th>
                            </tr>
                        </thead>
                        <tbody class="fw-semibold text-gray-600">
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Emma Smith</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">smith@kpmg.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-info">Disabled</div>
                                </td>
                                <td>15 Apr 2024, 6:05 pm</td>
                                <td class="text-end pe-0">40</td>
                                <td class="text-end pe-0">47</td>
                                <td class="text-end">$2645.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Melody Macy</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">melody@altbox.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>21 Feb 2024, 10:30 am</td>
                                <td class="text-end pe-0">15</td>
                                <td class="text-end pe-0">27</td>
                                <td class="text-end">$2497.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details" class="text-gray-900 text-hover-primary">Max
                                        Smith</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">max@kt.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-danger">Banned</div>
                                </td>
                                <td>20 Dec 2024, 5:20 pm</td>
                                <td class="text-end pe-0">76</td>
                                <td class="text-end pe-0">87</td>
                                <td class="text-end">$4267.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Sean Bean</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">sean@dellito.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-info">Disabled</div>
                                </td>
                                <td>25 Jul 2024, 5:20 pm</td>
                                <td class="text-end pe-0">8</td>
                                <td class="text-end pe-0">20</td>
                                <td class="text-end">$581.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Brian Cox</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">brian@exchange.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>20 Dec 2024, 10:10 pm</td>
                                <td class="text-end pe-0">49</td>
                                <td class="text-end pe-0">58</td>
                                <td class="text-end">$1672.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Mikaela Collins</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">mik@pex.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-info">Disabled</div>
                                </td>
                                <td>20 Jun 2024, 5:20 pm</td>
                                <td class="text-end pe-0">88</td>
                                <td class="text-end pe-0">98</td>
                                <td class="text-end">$4117.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Francis Mitcham</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">f.mit@kpmg.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>25 Oct 2024, 10:10 pm</td>
                                <td class="text-end pe-0">60</td>
                                <td class="text-end pe-0">67</td>
                                <td class="text-end">$4882.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Olivia Wild</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">olivia@corpmail.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>10 Nov 2024, 2:40 pm</td>
                                <td class="text-end pe-0">71</td>
                                <td class="text-end pe-0">80</td>
                                <td class="text-end">$2741.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Neil Owen</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">owen.neil@gmail.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>24 Jun 2024, 5:20 pm</td>
                                <td class="text-end pe-0">58</td>
                                <td class="text-end pe-0">66</td>
                                <td class="text-end">$4614.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Dan
                                        Wilson</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">dam@consilting.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>10 Mar 2024, 11:30 am</td>
                                <td class="text-end pe-0">99</td>
                                <td class="text-end pe-0">111</td>
                                <td class="text-end">$4553.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Emma Bold</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">emma@intenso.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>25 Jul 2024, 10:30 am</td>
                                <td class="text-end pe-0">35</td>
                                <td class="text-end pe-0">40</td>
                                <td class="text-end">$2005.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Ana
                                        Crown</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">ana.cf@limtel.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>10 Nov 2024, 11:30 am</td>
                                <td class="text-end pe-0">89</td>
                                <td class="text-end pe-0">101</td>
                                <td class="text-end">$2930.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Robert Doe</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">robert@benko.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>10 Mar 2024, 5:30 pm</td>
                                <td class="text-end pe-0">48</td>
                                <td class="text-end pe-0">61</td>
                                <td class="text-end">$1826.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">John Miller</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">miller@mapple.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>25 Oct 2024, 5:20 pm</td>
                                <td class="text-end pe-0">12</td>
                                <td class="text-end pe-0">22</td>
                                <td class="text-end">$4158.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Lucy Kunic</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">lucy.m@fentech.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>24 Jun 2024, 5:20 pm</td>
                                <td class="text-end pe-0">19</td>
                                <td class="text-end pe-0">28</td>
                                <td class="text-end">$3304.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Ethan Wilder</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">ethan@loop.com.au</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>15 Apr 2024, 11:30 am</td>
                                <td class="text-end pe-0">35</td>
                                <td class="text-end pe-0">44</td>
                                <td class="text-end">$157.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Brian Cox</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">brian@exchange.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-info">Disabled</div>
                                </td>
                                <td>15 Apr 2024, 11:30 am</td>
                                <td class="text-end pe-0">73</td>
                                <td class="text-end pe-0">83</td>
                                <td class="text-end">$674.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Emma Smith</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">smith@kpmg.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>19 Aug 2024, 9:23 pm</td>
                                <td class="text-end pe-0">34</td>
                                <td class="text-end pe-0">40</td>
                                <td class="text-end">$3620.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Melody Macy</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">melody@altbox.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-danger">Banned</div>
                                </td>
                                <td>10 Nov 2024, 6:43 am</td>
                                <td class="text-end pe-0">26</td>
                                <td class="text-end pe-0">40</td>
                                <td class="text-end">$1176.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Max
                                        Smith</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">max@kt.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>20 Jun 2024, 5:30 pm</td>
                                <td class="text-end pe-0">56</td>
                                <td class="text-end pe-0">70</td>
                                <td class="text-end">$2134.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Sean Bean</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">sean@dellito.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>05 May 2024, 2:40 pm</td>
                                <td class="text-end pe-0">3</td>
                                <td class="text-end pe-0">11</td>
                                <td class="text-end">$2917.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Brian Cox</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">brian@exchange.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>25 Oct 2024, 8:43 pm</td>
                                <td class="text-end pe-0">1</td>
                                <td class="text-end pe-0">13</td>
                                <td class="text-end">$2173.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Mikaela Collins</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">mik@pex.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>19 Aug 2024, 10:30 am</td>
                                <td class="text-end pe-0">65</td>
                                <td class="text-end pe-0">76</td>
                                <td class="text-end">$3593.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Francis Mitcham</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">f.mit@kpmg.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>19 Aug 2024, 11:05 am</td>
                                <td class="text-end pe-0">62</td>
                                <td class="text-end pe-0">68</td>
                                <td class="text-end">$2117.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Olivia Wild</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">olivia@corpmail.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>25 Jul 2024, 11:05 am</td>
                                <td class="text-end pe-0">81</td>
                                <td class="text-end pe-0">94</td>
                                <td class="text-end">$4385.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Neil Owen</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">owen.neil@gmail.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>24 Jun 2024, 10:10 pm</td>
                                <td class="text-end pe-0">58</td>
                                <td class="text-end pe-0">69</td>
                                <td class="text-end">$2165.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Dan
                                        Wilson</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">dam@consilting.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>20 Dec 2024, 5:30 pm</td>
                                <td class="text-end pe-0">9</td>
                                <td class="text-end pe-0">20</td>
                                <td class="text-end">$560.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Emma Bold</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">emma@intenso.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>25 Jul 2024, 5:20 pm</td>
                                <td class="text-end pe-0">59</td>
                                <td class="text-end pe-0">71</td>
                                <td class="text-end">$3198.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Ana
                                        Crown</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">ana.cf@limtel.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>25 Oct 2024, 6:43 am</td>
                                <td class="text-end pe-0">11</td>
                                <td class="text-end pe-0">22</td>
                                <td class="text-end">$1391.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Robert Doe</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">robert@benko.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>19 Aug 2024, 8:43 pm</td>
                                <td class="text-end pe-0">98</td>
                                <td class="text-end pe-0">113</td>
                                <td class="text-end">$4461.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">John Miller</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">miller@mapple.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-warning">
                                        Locked
                                    </div>
                                </td>
                                <td>20 Jun 2024, 9:23 pm</td>
                                <td class="text-end pe-0">61</td>
                                <td class="text-end pe-0">76</td>
                                <td class="text-end">$4153.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Lucy Kunic</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">lucy.m@fentech.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>21 Feb 2024, 9:23 pm</td>
                                <td class="text-end pe-0">73</td>
                                <td class="text-end pe-0">86</td>
                                <td class="text-end">$2019.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Ethan Wilder</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">ethan@loop.com.au</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>25 Jul 2024, 9:23 pm</td>
                                <td class="text-end pe-0">20</td>
                                <td class="text-end pe-0">32</td>
                                <td class="text-end">$2375.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Ana
                                        Crown</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">ana.cf@limtel.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>15 Apr 2024, 5:20 pm</td>
                                <td class="text-end pe-0">28</td>
                                <td class="text-end pe-0">39</td>
                                <td class="text-end">$4072.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Emma Smith</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">smith@kpmg.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>24 Jun 2024, 8:43 pm</td>
                                <td class="text-end pe-0">91</td>
                                <td class="text-end pe-0">102</td>
                                <td class="text-end">$1789.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Melody Macy</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">melody@altbox.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>10 Nov 2024, 5:30 pm</td>
                                <td class="text-end pe-0">30</td>
                                <td class="text-end pe-0">40</td>
                                <td class="text-end">$644.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Max
                                        Smith</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">max@kt.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>20 Jun 2024, 9:23 pm</td>
                                <td class="text-end pe-0">17</td>
                                <td class="text-end pe-0">31</td>
                                <td class="text-end">$1869.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Sean Bean</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">sean@dellito.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>22 Sep 2024, 6:43 am</td>
                                <td class="text-end pe-0">3</td>
                                <td class="text-end pe-0">9</td>
                                <td class="text-end">$1607.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Brian Cox</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">brian@exchange.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>21 Feb 2024, 2:40 pm</td>
                                <td class="text-end pe-0">10</td>
                                <td class="text-end pe-0">20</td>
                                <td class="text-end">$4546.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Mikaela Collins</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">mik@pex.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>20 Dec 2024, 8:43 pm</td>
                                <td class="text-end pe-0">64</td>
                                <td class="text-end pe-0">76</td>
                                <td class="text-end">$1721.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Francis Mitcham</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">f.mit@kpmg.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>20 Jun 2024, 11:30 am</td>
                                <td class="text-end pe-0">87</td>
                                <td class="text-end pe-0">94</td>
                                <td class="text-end">$4989.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Olivia Wild</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">olivia@corpmail.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>25 Oct 2024, 5:30 pm</td>
                                <td class="text-end pe-0">34</td>
                                <td class="text-end pe-0">48</td>
                                <td class="text-end">$3155.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Neil Owen</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">owen.neil@gmail.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>25 Jul 2024, 8:43 pm</td>
                                <td class="text-end pe-0">97</td>
                                <td class="text-end pe-0">111</td>
                                <td class="text-end">$602.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Dan
                                        Wilson</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">dam@consilting.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>24 Jun 2024, 5:20 pm</td>
                                <td class="text-end pe-0">34</td>
                                <td class="text-end pe-0">43</td>
                                <td class="text-end">$3739.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Emma Bold</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">emma@intenso.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>24 Jun 2024, 6:05 pm</td>
                                <td class="text-end pe-0">43</td>
                                <td class="text-end pe-0">54</td>
                                <td class="text-end">$3414.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Ana
                                        Crown</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">ana.cf@limtel.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>19 Aug 2024, 5:20 pm</td>
                                <td class="text-end pe-0">75</td>
                                <td class="text-end pe-0">84</td>
                                <td class="text-end">$2690.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Robert Doe</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">robert@benko.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>24 Jun 2024, 5:30 pm</td>
                                <td class="text-end pe-0">71</td>
                                <td class="text-end pe-0">80</td>
                                <td class="text-end">$2194.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">John Miller</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">miller@mapple.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>20 Jun 2024, 10:30 am</td>
                                <td class="text-end pe-0">100</td>
                                <td class="text-end pe-0">108</td>
                                <td class="text-end">$1620.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Lucy Kunic</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">lucy.m@fentech.com</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-danger">Banned</div>
                                </td>
                                <td>10 Mar 2024, 11:30 am</td>
                                <td class="text-end pe-0">90</td>
                                <td class="text-end pe-0">96</td>
                                <td class="text-end">$3113.00</td>
                            </tr>
                            <tr>
                                <td>
                                    <a href="/apps/ecommerce/customers/details"
                                        class="text-gray-900 text-hover-primary">Ethan Wilder</a>
                                </td>
                                <td>
                                    <a href="javascript:void(0)" class="text-gray-900 text-hover-primary">ethan@loop.com.au</a>
                                </td>
                                <td>
                                    <div class="badge badge-light-success">
                                        Active
                                    </div>
                                </td>
                                <td>21 Feb 2024, 6:43 am</td>
                                <td class="text-end pe-0">96</td>
                                <td class="text-end pe-0">102</td>
                                <td class="text-end">$137.00</td>
                            </tr>
                        </tbody>
                    </table>
                    <!--end::Table-->
                </div>
                <!--end::Card body-->
            </div>
            <!--end::Products-->
        </div>
        <!--end::Content container-->
    </div>
    <!--end::Content-->
@endsection
@section('scripts')
    <!--begin::Vendors Javascript(used for this page only)-->
    <script src="{{ \App\Support\ThemeAsset::url('plugins/custom/datatables/datatables.bundle.js', $theme_asset_pack ?? null) }}"></script>
    <!--end::Vendors Javascript-->
    <!--begin::Custom Javascript(used for this page only)-->
    <script src="{{ \App\Support\ThemeAsset::url('js/custom/apps/ecommerce/reports/customer-orders/customer-orders.js', $theme_asset_pack ?? null) }}"></script>
    <script src="{{ \App\Support\ThemeAsset::url('js/widgets.bundle.js', $theme_asset_pack ?? null) }}"></script>
    <script src="{{ \App\Support\ThemeAsset::url('js/custom/widgets.js', $theme_asset_pack ?? null) }}"></script>
    <script src="{{ \App\Support\ThemeAsset::url('js/custom/apps/chat/chat.js', $theme_asset_pack ?? null) }}"></script>
    <script src="{{ \App\Support\ThemeAsset::url('js/custom/utilities/modals/upgrade-plan.js', $theme_asset_pack ?? null) }}"></script>
    <script src="{{ \App\Support\ThemeAsset::url('js/custom/utilities/modals/create-app.js', $theme_asset_pack ?? null) }}"></script>
    <script src="{{ \App\Support\ThemeAsset::url('js/custom/utilities/modals/users-search.js', $theme_asset_pack ?? null) }}"></script>
    <!--end::Custom Javascript-->
@endsection
