...
<div class="card card-flush shadow-sm border-0 mb-6">
    <div class="card-header pt-6 px-4 px-md-6 flex-column flex-sm-row align-items-start align-items-sm-center justify-content-between gap-3">
        <div class="card-title">
            <i class="ki-outline ki-tablet text-primary fs-2 me-2" id="kt_preview_icon"></i>
            <div>
                <h3 class="fw-bolder text-gray-900 m-0 fs-4" id="kt_preview_heading">Pratinjau Langsung (Live Interactive Preview)</h3>
                <span class="text-muted fs-7" id="kt_preview_subheading">
                    @if(($currentFrontpage ?? 'landing') === 'education')
                        Uji tampilan portal akademik Education langsung dalam berbagai mode layar.
                    @else
                        Uji tampilan Landing Page langsung dalam berbagai mode layar.
                    @endif
                </span>
            </div>
        </div>
        <div class="card-toolbar d-flex flex-wrap gap-2 w-100 w-sm-auto justify-content-start justify-content-sm-end">
            ...
            <a href="{{ url(($currentFrontpage ?? 'landing') === 'education' ? '/education' : '/landing') }}" target="_blank" class="btn btn-light-primary btn-sm fw-bold" id="kt_btn_preview_open_tab">
    ...
    <div class="card-body p-2 p-md-4 bg-light-subtle d-flex justify-content-center overflow-hidden">
        <div id="kt_preview_container" style="width: 100%; transition: width 0.3s ease; border-radius: 12px; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.1); background: #fff;">
            <iframe id="kt_preview_iframe" src="{{ url(($currentFrontpage ?? 'landing') === 'education' ? '/education' : '/landing') }}" style="width: 100%; height: 750px; border: none;"></iframe>
        </div>
    </div>
</div>
