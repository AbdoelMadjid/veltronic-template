    ...
    <div class="{{ $vars['body_class'] ?? 'card-body d-flex justify-content-between flex-column px-0 pb-0' }}">
        ...
        <div class="{{ $vars['stats_class'] ?? 'mb-4 px-9' }}">
            ...
            <div class="{{ $vars['info_row_class'] ?? 'd-flex align-items-center mb-2' }}">
                <!--begin::Value-->
                <span class="{{ $vars['value_class'] ?? 'fs-2hx fw-bold text-gray-800 me-2 lh-1' }}">{{ $vars['value'] ?? '29,420' }}</span>
                <!--end::Value-->
                <!--begin::Label-->
            ...
            <!--end::Statistics-->
            <!--begin::Description-->
            <span class="{{ $vars['description_class'] ?? 'fs-6 fw-semibold text-gray-500' }}">{{ $vars['description'] ?? 'Total Online Visitors' }}</span>
            <!--end::Description-->
        </div>
        <!--end::Statistics-->
        <!--begin::Chart-->
        <div id="{{ $vars['chart_id'] ?? 'kt_card_widget_9_chart' }}" class="{{ $vars['chart_class'] ?? 'min-h-auto' }}"
