...
<div class="{{ $vars['card_class'] ?? 'card card-flush h-xl-100' }}">
    ...
    <div class="{{ $vars['header_class'] ?? 'card-header pt-7' }}">
        ...
        <h3 class="{{ $vars['title_wrapper_class'] ?? 'card-title align-items-start flex-column' }}">
            <span class="{{ $vars['title_class'] ?? 'card-label fw-bold text-gray-900' }}">{{ $vars['title'] ?? 'Departments' }}</span>
            <span class="{{ $vars['subtitle_class'] ?? 'text-gray-500 pt-2 fw-semibold fs-6' }}">{{ $vars['subtitle'] ?? 'Performance & achievements' }}</span>
        </h3>
        <!--end::Title-->
        <!--begin::Toolbar-->
                ...
                <div class="menu-item px-3">
                    <a href="javascript:void(0)" class="menu-link flex-stack px-3">Create Payment
                        <span class="ms-2" data-bs-toggle="tooltip"
                            ...
                            <i class="{{ $vars['info_icon_class'] ?? 'ki-outline ki-information fs-6' }}">
                                @if ($vars['info_icon_duotone'] ?? false)
                                    <span class="path1"></span>
                                    <span class="path2"></span>
                                    <span class="path3"></span>
                                @endif
    ...
    <div class="{{ $vars['body_class'] ?? 'card-body pt-5' }}">
        ...
        <div id="{{ $vars['chart_id'] ?? 'kt_charts_widget_14_chart' }}" class="{{ $vars['chart_class'] ?? 'w-100 h-350px' }}"></div>
        <!--end::Chart container-->
    </div>
    <!--end::Body-->
</div>
