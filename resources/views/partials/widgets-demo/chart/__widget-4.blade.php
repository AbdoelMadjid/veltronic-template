...
<div class="{{ $vars['card_class'] ?? 'card card-flush overflow-hidden h-md-100' }}">
    ...
    <div class="{{ $vars['header_class'] ?? 'card-header py-5' }}">
        ...
        <h3 class="{{ $vars['title_wrapper_class'] ?? 'card-title align-items-start flex-column' }}">
            <span class="{{ $vars['title_class'] ?? 'card-label fw-bold text-gray-900' }}">{{ $vars['title'] ?? 'Discounted Product Sales' }}</span>
    ...
    <div class="{{ $vars['body_class'] ?? 'card-body d-flex justify-content-between flex-column pb-1 px-0' }}">
        ...
        <div class="{{ $vars['stats_wrapper_class'] ?? 'px-9 mb-5' }}">
            <!--begin::Statistics-->
            <div class="{{ $vars['stats_row_class'] ?? 'd-flex align-items-center mb-2' }}">
                <!--begin::Currency-->
                <span class="{{ $vars['currency_class'] ?? 'fs-4 fw-semibold text-gray-500 align-self-start me-1' }}">{{ $vars['currency'] ?? '$' }}</span>
                <!--end::Currency-->
                <!--begin::Value-->
                <span class="{{ $vars['amount_class'] ?? 'fs-2hx fw-bold text-gray-800 me-2 lh-1 ls-n2' }}">{{ $vars['amount'] ?? '3,706' }}</span>
                <!--end::Value-->
                <!--begin::Label-->
                <span class="{{ $vars['badge_class'] ?? 'badge badge-light-success fs-base' }}">
                    <i class="{{ $vars['badge_icon_class'] ?? 'ki-outline ki-arrow-down fs-5 text-success ms-n1' }}">
                        @if ($vars['badge_icon_duotone'] ?? false)
                            <span class="path1"></span>
                            <span class="path2"></span>
                        @endif
                    </i>{{ $vars['badge_text'] ?? '4.5%' }}</span>
                <!--end::Label-->
            </div>
            <!--end::Statistics-->
            <!--begin::Description-->
            <span class="{{ $vars['description_class'] ?? 'fs-6 fw-semibold text-gray-500' }}">{{ $vars['description'] ?? 'Total Discounted Sales This Month' }}</span>
            <!--end::Description-->
        </div>
        <!--end::Info-->
        <!--begin::Chart-->
        <div id="{{ $vars['chart_id'] ?? 'kt_charts_widget_4' }}" class="{{ $vars['chart_class'] ?? 'min-h-auto ps-4 pe-6' }}"
